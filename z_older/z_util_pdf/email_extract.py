...
def _save_summary_info(data_frame):
    ...
    try:
        with pd.ExcelWriter(config.read("EMAIL_SUMMARY"),mode='a',if_sheet_exists="replace") as writer:  
...
def capture_email_count_complexity(OUTLOOK):
    ...
    df = pd.DataFrame()
    df.to_excel(config.read("EMAIL_SUMMARY"),sheet_name='snapshot',index=False)
